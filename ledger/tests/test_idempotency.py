    ...
    def test_duplicate_key_rejected_unique_keys_succeed(self, mock_task):
        ...
        for key1, key2, should_reject_second in cases:
            with self.subTest(key1=key1, key2=key2):
                acct = AccountFactory(user=self.user, balance=Decimal("1000.0000"))
                p1 = {"account": str(acct.id), "amount": "100.00", "idempotency_key": key1}
                p2 = {"account": str(acct.id), "amount": "100.00", "idempotency_key": key2}
